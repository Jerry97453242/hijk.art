...
getVersion() {
    ...
    NEW_VER=$(curl -s https://api.github.com/repos/XTLS/Xray-core/releases | jq -r '.[]|select (.prerelease=='${prereleaseStatus}')|.tag_name' | head -1)
...
getData() {
    if [[ "$TLS" = "true" || "$XTLS" = "true" ]]; then
	    ...
	    colorEcho ${BLUE}  "已默认${DOMAIN}设置成解析到本机"
